...
- if the original dataset contains species details across two columns (*species* and *genus*), data in the columns are concatinated in the form `"genus_species"` and merged into a single `$species` column. ***ensure genus column name in files is matched to `"genus"` in `vnames`.*** 
...
- to source for across *all files* in file.system matching the `file.name`: name script as `file.name`. eg .`"Amniote_Database_Aug_2015.R"`
- to source for *individual files* in file.system matching the `file.name`: name script as `file.name` append with appropriate `fcode`, eg .`"Amniote_Database_Aug_2015_ref.R"`
